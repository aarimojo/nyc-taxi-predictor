...
# def predict_trip(trip: TripRequest):
#     """
#     Predict the trip duration and fare amount.
#     """
#     duration, fare = predictor.predict(
...
#     return {"duration": float(duration), "fare": float(fare)}
...
#         return {"duration": round(duration, 2), "fare": round(fare, 2)}
...
async def predict(input_data: dict):
    try:
        predictor = DummyTaxiPredictwor()  # Cambiar a DummyTaxiPredictor para testing
        # predictor = TaxiPredictor() 
        duration = predictor.predict_duration(input_data)
        fare = predictor.predict_fare(input_data)
        return {
